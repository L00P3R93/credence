<x-filament-panels::page>
    ...
    {{-- Summary --}}
    @php $s = $this->getDisbursementSummary(); @endphp
    <x-filament::section heading="Disbursement Summary">
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 1.5rem;">
            @foreach ([
                ...
                ['label' => 'Refinanced', 'count' => $s['refinanced']['count'], 'total' => $s['refinanced']['total']],
            ] as $stat)
                <div>
            ...
            @endforeach
            <div style="border-left: 2px solid currentColor; opacity: 0.15;"></div>
            <div>
                <div style="font-size: 11px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.06em; opacity: 0.55;">Grand Total</div>
                <div style="font-size: 22px; font-weight: 700; margin-top: 4px;">{{ number_format($s['grand_total']['total']) }}</div>
                <div style="font-size: 12px; opacity: 0.55; margin-top: 2px;">
                    {{ $s['grand_total']['count'] }} loans &middot; Interest: {{ number_format($s['grand_total']['interest']) }}
                </div>
            </div>
        </div>
    </x-filament::section>
