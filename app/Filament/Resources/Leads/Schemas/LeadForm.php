        ...
        return $schema->components([
            ...
            Section::make('Loan Product Details')->schema([
                ...
                Select::make('bank_branch_id')
                    ->label('Bank Branch')
                    ->options(function (callable $get, ?Lead $record) {
                        $bankId = $get('bank_id');
                        if (!$bankId && $record) {
                    ...
                    ->native(false)
                    ->required()
                    ->visible(fn (callable $get, ?Lead $record) => $get('bank_id') !== null || ($record && $record->bank_id))
                    ->searchable()
                    ->preload()
            ...
            ])->columns(fn (callable $get, ?Lead $record) => $get('bank_id') !== null || ($record && $record->bank_id) ? 3 : 2)->columnSpanFull(),
